...
关于 Matrix 引擎更详细的说明，参见[这里](TransMatrixAPI文档/3_接口说明/Matrix/matrix.md)。为了使策略程序按照我们的预期思路运行，我们需要对 Matrix 引擎作如下配置：
...
如上所示，这里，我们将 mode 设置为 simulation，代表当前模式为策略研究。TransMatrix 支持2种研究模式：1是策略研究，2是因子研究。因子研究模式，相关介绍参见[第五章](TransMatrix使用手册/5_开展因子研究/signal.md)，策略研究模式需要将 mode 设为 simulation。
...
```
    ...
    这里，我们订阅的是 meta_data数据库里的future_bar_10min表，该表存放了期货10分钟的 Bar 行情数据。关于数据库 API 的详细说明，参见说明文档[数据库](TransMatrix使用手册/7_数据库/database.md)部分，关于 DataApi 的详细说明，参见说明文档[DataApi](TransMatrixAPI文档/3_接口说明/数据模型/set_model_view.md) 部分。
...
k 线撮合器是系统所支持的一种常用的撮合器，此外，系统还支持 日频 / 快照 与 Orderflow 撮合成交。开发者可根据特定场景开发自己的撮合器，只需要重载撮合器基类 Matcher的几个方法即可实现。撮合器的具体说明参见[这里](TransMatrixAPI文档/4_其他组件/market_components.md)。
...
策略继承自 Strategy 类，所有的回测策略，都要继承该类，以实现自身的策略逻辑。关于策略组件基类 Strategy 的详细说明，参见[这里](TransmatrixAPI文档/3_接口说明/策略/strategy.md)。
...
>
> 用户也可以添加调度器，实现自定义的回调需求，调度器的更多说明，参见[这里](TransMatrix使用手册/4_开展策略研究/simulation.md?id=_26-一个股票策略示例)。
...
关于 评价组件基类 Evaluator 的详细说明，参见[这里](TransMatrixAPI文档/3_接口说明/评价/evaluator.md)。
...
前文我们以一个期货 CTA 策略和一个股票 CTA 策略为示例，详细介绍了如何使用 TransMatrix 系统开展策略研究。事实上，TransMatrix 支持各种类型的策略回测，包括但不限于：股票 T0、期货做市、ETF 套利、可转债高频等等。系统也提供了不同类型的策略示例，参见[这里](TransMatrixAPI文档/8_测例代码/index.md)。
<div align=center>
