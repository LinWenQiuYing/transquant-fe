...
export default function VersionModal(props: VersionModalProps) {
  ...
  const {
    ...
    switchJobDefinitionVersions,
    deleteJobDefinitionVersions,
  ...
  const getVersions = (pageNo: number) => {
    getJobDefinitionVersions(code, pageNo).then((res: any) => {
  ...
  const onSwitchVersion = (versionId: number) => {
    switchJobDefinitionVersions(code, versionId).then(() => {
      getVersions(currentPage);
      message.success("切换成功");
    });
  };
  const onDeleteVersion = (versionId: number) => {
    deleteJobDefinitionVersions(code, versionId).then(() => {
      getVersions(currentPage);
        ...
        <Space size="middle">
          <Tooltip title="切换到该版本">
            <Typography.Link
              ...
              disabled={version === record.version}
            >
