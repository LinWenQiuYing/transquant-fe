...
export default observer(function GroupStrategy() {
  const { collapsed } = useStores().appStore;
  const { strategyBaseInfo } = useSpaceStores().groupStrategyStore;
  ...
  const getBreadcrumbItems = () => {
    ...
    if ("id" in params) {
      const path = `${paths.groupStrategy}/${params.id}`;
  ...
  return (
    <Permission code="groupStrategy">
      <ContentPanel
        className={`${clsPrefix}-strategy-group`}
        title={
          <TitlePanel
