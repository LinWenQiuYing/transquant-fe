...
export default observer(function StrategyModal<
...
>(props: FactorModalProps<T>) {
  ...
  const {
    ...
    allTags,
    userList,
    downloadTeamStrategyProject,
    updateTeamStrategyProject,
  } = useStores().groupStrategyStore;

  const [form] = Form.useForm();
  const [formValues, setFormValues] = useState(defaultFormValues);
  const isEdit = title === "编辑策略项目";
  ...
  const onOk = async () => {
    form.validateFields().then(async (values) => {
      form.resetFields();
      if (isEdit) {
        await updateTeamStrategyProject({
          ...values,
          projectId: data?.id,
        });
      } else {
        await downloadTeamStrategyProject({
          ...values,
