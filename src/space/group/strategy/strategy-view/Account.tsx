...
const SearchHeader = observer(() => {
  const {
    ...
    onStrategyViewAccountSearchValueChange,
    currentJob,
  ...
  const onSearch = (config: Partial<IAccountSearch>) => {
    getTeamAccountDetail({ ...config, strategyId });
...
export default observer(function AccountView() {
  const { accountTable, accountTableLoading } = useStores().groupStrategyStore;

  const columns: ColumnsType<DataType<AccountTable>> = [
    {
      title: "日期",
      dataIndex: "datetime",
      key: "dateTime",
  ...
  return (
    <div>
      <Table
        columns={columns}
        dataSource={useDataSource<AccountTable>(accountTable)}
        size="small"
