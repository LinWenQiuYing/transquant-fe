...
export default observer(function FactorViewContent() {
  ...
  const onChange = (key: string) => {
    ...
    switch (key) {
      case FactorViewTabEnum.Detail:
        getTeamStrategyJsonData(strategyId);
        break;
      ...
      case FactorViewTabEnum.Account:
        getTeamAccountDetail({ strategyId, timeRangeType: 0 });
        break;
  ...
  const onDetailExport = () => {
    const element = document.querySelector<HTMLElement>(
      "#container-group .trans-quant-antd-spin-container"
    );
    if (!element) {
      return;
    }
    ...
    },
    {
      ...
      children: (
        <PerformanceView
          ...
          data={JSON.parse(groupStrategyPerformance).data}
        />
      ),
    },
  ];
