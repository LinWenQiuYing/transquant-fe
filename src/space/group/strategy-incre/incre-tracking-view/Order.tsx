...
const SearchHeader = observer(() => {
  ...
  const onSearch = (config: Partial<IncreOrderSearch>) => {
    const _config = { pageNum: 1, pageSize: pagination.pageSize, ...config };
    getOrdersList(_config);
...
export default observer(function OrderView() {
  ...
  const columns: ColumnsType<DataType<IOrder>> = [
    {
      title: "时间",
      dataIndex: "datetime",
      key: "dateTime",
  ...
  return (
    <div>
      <Table
        columns={columns}
        dataSource={useDataSource<IOrder>(orders?.datas)}
        size="small"
        ...
        pagination={{
          ...usePagination({
            ...
            onRequest(pageIndex, pageSize) {
              getOrdersList({ pageNum: pageIndex, pageSize, timeRangeType: 0 });
            },
          }),
        }}
        title={() => <SearchHeader />}
