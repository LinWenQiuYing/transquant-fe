...
import { Col, Empty, Row, Space, Spin, Statistic } from "antd";
import { observer } from "mobx-react";
import { useEffect } from "react";
import { useStores } from "../../../hooks";
import { StrategyKPI, StrategyPnl, StrategyReturn } from "../../../types";
...
const StrategyReturnView = (props: {
  strategyReturnList: StrategyReturn[];
}) => {
  const { strategyReturnList } = props;
...
const StrategyPnlView = (props: { strategyPnlList: StrategyPnl[] }) => {
  const { strategyPnlList } = props;
...
export default observer(function ProfitView() {
  const {
    strategyKPI,
    strategyPnlList,
    strategyPnlListLoading,
    strategyReturnList,
    strategyReturnListLoading,
  } = useStores().groupStrategyStore;

  return (
    <Space direction="vertical" style={{ width: "100%" }} size={20}>
      <ProfitKPI strategyKPI={strategyKPI} />
      <Spin spinning={strategyReturnListLoading} style={{ height: 300 }}>
        {!strategyReturnListLoading && (
          <StrategyReturnView strategyReturnList={strategyReturnList} />
        )}
      </Spin>
      <Spin
        spinning={strategyPnlListLoading}
        style={{ height: 300, marginTop: 300 }}
      >
        {!strategyPnlListLoading && (
          <StrategyPnlView strategyPnlList={strategyPnlList} />
        )}
      </Spin>
    </Space>
  );
