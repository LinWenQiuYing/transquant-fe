...
import { DataType, thousandthFormat } from "@transquant/utils";
...
import { Job, StrategyJob } from "../../../types";
import "./index.less";
import OperatorMenu from "./OperatorMenu";
import SearchHeader from "./SearchHeader";

export const IncreTrackingStatus = {
  0: "生效中",
  1: "已停止",
  2: "已结束",
  3: "未启动",
...
export default observer(function IncreTracking() {
  const { jobs, strategyQuartzJobLoading } = useStores().groupStrategyStore;
  ...
  const columns: ColumnsType<DataType<Job>> = [
    ...
    },
    {
      ...
      width: 200,
      render(value) {
        return <span>{value ? thousandthFormat(value) : ""}</span>;
      },
    },
    {
      title: "收益率",
      ...
      width: 200,
      render(value) {
        return <span>{value ? `${(value * 100).toFixed(2)}%` : ""}</span>;
      },
    },
    {
      title: "操作",
  ...
  return (
    <Table
      ...
      dataSource={useDataSource<StrategyJob>(jobs)}
      size="small"
      loading={strategyQuartzJobLoading}
