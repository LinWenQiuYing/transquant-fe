...
export default observer(function FactorView() {
  const { facotrSumList, factorPieChart } = useStores().homeStore;

  const total = factorPieChart.reduce((sum, curr) => {
    sum += curr.sum;
    return sum;
  }, 0);
  ...
  const pieRadioOptions: ECOption = {
    ...
    legend: {
      ...
      formatter(name) {
        let total = 0;
        let target = 0;
        for (let i = 0; i < factorPieChart.length; i++) {
          total += factorPieChart[i].sum;
          if (factorPieChart[i].teamName === name) {
            target = factorPieChart[i].sum;
          }
        }
        const arr = [
  ...
  const lineFactorOptions: ECOption = {
    ...
    xAxis: {
      ...
      data: getAxias(facotrSumList, "teamLineChartList"),
    },
    axisTick: {
      lineStyle: {
        color: grey400,
    ...
    series: facotrSumList.map((item, index) => {
  ...
  const _pieRadioOptions = factorPieChart.length
    ? pieRadioOptions
    : pieEmptyRadioOptions;
  const _lineFactorOptions = facotrSumList.length
    ? lineFactorOptions
  ...
  const { domRef: activeRadioRef, update: activeRadioRefUpdate } =
    useEcharts<HTMLDivElement>(lineFactorOptions);
  ...
  }, [facotrSumList, factorPieChart]);

  return (
    <div className={`${clsPrefix}-home-team-factor`}>
