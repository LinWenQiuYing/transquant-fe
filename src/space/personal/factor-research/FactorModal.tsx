...
export default observer(function FactorModal<T extends DataType<ProjectItem>>(
  props: FactorModalProps<T>
) {
  const { title, visible, onVisibleChange, data } = props;
  const { createFactorProject, updateFactorProject, templates, allTags } =
  ...
  const onOk = async () => {
    form.validateFields().then(async (values) => {
      ...
      if (isEdit) {
        await updateFactorProject({ ...values, projectId: data?.id });
      } else {
        await createFactorProject(values);
      }
