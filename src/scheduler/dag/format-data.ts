...
export function formatParams(data: INodeData): {
  ...
  const rdbmsSourceTypes = ["MYSQL", "ORACLE", "SQLSERVER", "HANA"];
  ...
  if (data.taskType === "SQOOP") {
    ...
    if (data.isCustomTask) {
      ...
      if (data.targetType === "HIVE") {
        ...
        rdbmsSourceTypes.some((target) => target === data.targetType)
      ) {
        targetParams = {
      ...
      if (rdbmsSourceTypes.some((target) => target === data.sourceType)) {
        sourceParams = {
